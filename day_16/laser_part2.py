...
for x in range(x_lim+1):
    starts.append(PathPart(Position(x, 0), 'V'))
    starts.append(PathPart(Position(x, y_lim), '^'))
for y in range(y_lim+1):
    starts.append(PathPart(Position(0, y), '>'))
    starts.append(PathPart(Position(x_lim, y), '<'))

length = len(starts)
for i, start in enumerate(starts, start=1):
    print(f"{i:03d} / {length:03} : {start}")
